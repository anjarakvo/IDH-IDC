...
from models.case import (
    Case, CaseBase, CaseDict, CaseListDict
)
...
def add_case(
    session: Session, payload: CaseBase, user: User
) -> CaseDict:
    case = Case(
        name=payload.name,
        date=payload.date,
    ...
    def_case_commodity = CaseCommodity(
        commodity=payload.focus_commodity, breakdown=1)
...
def get_all_case(
    ...
    if tags:
        case_tags = session.query(CaseTag).filter(
            CaseTag.tag.in_(tags)).all()
        case_ids = [pt.case for pt in case_tags]
        case = case.filter(Case.id.in_(case_ids))
    count = case.count()
    case = case.order_by(
        Case.id.desc()).offset(skip).limit(limit).all()
...
def update_case(
    session: Session, id: int, payload: CaseBase
) -> CaseDict:
    ...
    if payload.other_commodities:
        for val in payload.other_commodities:
            breakdown = 1 if val.breakdown else 0
            prev_case_commodity = session.query(CaseCommodity).filter(and_(
